...
*/
public class CheckIfNumberIsAggregatedNumber {
    public static void test() {
        CheckIfNumberIsAggregatedNumber obj = new CheckIfNumberIsAggregatedNumber();
    ...
    public boolean isAggregated(String num) {
        for (int l1 = 1; l1 <= num.length() / 2; l1++) {
            for (int l2 = l1 + 1; l2 <= num.length(); l2++) {
                if (l2 < num.length() && isMatch(num, l1, l2))
                    return true;
            }
        }
    ...
    private boolean isMatch(String num, int l1, int l2) {
        ...
        while (l2 < num.length()) {
            ...
            if (l2 + sum.length() > num.length())
                return false;

            var next = num.substring(l2, l2 + sum.length());
            if (Integer.parseInt(sum) != Integer.parseInt(next))
                return false;
