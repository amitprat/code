...
class SurpassersOfArray {
   public:
    ...
    static void merge(vector<int>& arr, int l, int m, int r, vector<int>& tmp, vector<int>& arrIndex, vector<int>& surpassers) {
        ...
        while (i <= m || j <= r) {
            // if the right half already processed, the remaining elements in left are all smaller than the right elements. We can set the count of values
            // bigger on right.
            // if element on left > right, then we can set the cumulate surpasser values at this index as seen so far.
            // we compare the elements by original indices in array as merge sort will change the order of elements during sort.
            if (j > r || (i <= m && arr[arrIndex[i]] >= arr[arrIndex[j]])) {
                tmp[k++] = arrIndex[i];
                surpassers[arrIndex[i]] += acc;
                i++;
            } else {
                // if the element on right > element on left, incrrement the count
                tmp[k++] = arrIndex[j];
                acc++;
                j++;
            }
