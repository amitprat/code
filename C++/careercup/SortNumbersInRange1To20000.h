...
class SortNumbersInRange1To20000 {
   ...
   public:
    static void test() {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            obj.sortNumbersInRange(output, range);
        ...
        cout << format("Original Array = {}", to_string(input)) << endl;

        vector<int> output = input;
        obj.sortNumbersInRange(output, range);
        cout << format("Sorted Array = {}", to_string(output)) << endl;
    }

   private:
    void sortNumbersInRange(vector<int>& arr, int range) {
        Bitset bitset(range);
        for (auto e : arr) {
            bitset.add(e);
        }
   ...
   private:
    bool hasElements(vector<int>& input, vector<int>& output) {
        ...
        for (auto e : input)
            if (elements.find(e) == elements.end()) return false;
        return true;
    }
