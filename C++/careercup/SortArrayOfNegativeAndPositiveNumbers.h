...
class SortArrayOfNegativeAndPositiveNumbers {
   ...
   public:
    ...
    static void merge(vector<int>& arr, int l, int m, int r) {
        ...
        // find the starting index of first positive element.
        while (j1 <= m && arr[j1] < 0) j1++;
        while (j2 <= r && arr[j2] < 0) j2++;

        inverse(arr, j1, m);       // inverse of positive part of first array [A2`]
        inverse(arr, i2, j2 - 1);  //  inverse of negative part of second array [B1`]
        inverse(arr, j1, j2 - 1);  // inverse of [A2`B1`]` => [B1,A2]

        // after this overall becomes: [A1,B1,B2,B2]
    }
